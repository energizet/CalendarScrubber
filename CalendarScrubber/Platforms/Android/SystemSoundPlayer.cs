...
public class SystemSoundPlayer : ISystemSoundPlayer
{
	...
	public void Play()
	{
		try
		{
			...
			var alertUri = GetAlertUri();

			AppLogger.Log($"🎵 Используемый URI звука: {alertUri}");

			lock (_lock)
			{
				_mediaPlayer ??= CreatePlayer(alertUri);
			}

			if (_mediaPlayer.IsPlaying)
			{
				AppLogger.Log("▶️ SystemSoundPlayer: Уже воспроизводится");
				return;
			}
		...
		{
			AppLogger.Log($"❌ SystemSoundPlayer: Ошибка проигрывания звука {ex.Message}");
		}
	}

	private static Uri GetAlertUri()
	{
	...
	private MediaPlayer CreatePlayer(Uri alertUri)
	{
		var mediaPlayer = new MediaPlayer();
		mediaPlayer.SetDataSource(Application.Context, alertUri);
